...
export class AppComponent implements OnInit {
  ...
  public recents: Array<{show_id: string, show_name: string, poster_url: string}> | undefined;
  private currentSeasons = 0;
  private zoomYAxis = false;
  public errorMessage: string | null = null;
  public lastUpdated: any = null;
  public allShowData: any[] | undefined;
  public showSeasonsLengthArr: any[] | undefined;
  ...
  changeSeason(e) {
    ...
    if (filter === 'all') {
      ...
      this.renderChart(this.allShowData?.filter((episode: any) => episode.season === Number(filter)));
    }
  }
  ...
  fetchRecentsOnly(): void {
    this.myServiceService.getRecentsOnly().subscribe((res: any) => {
      ...
      // this.recents = a.map(i => JSON.parse(i));
    }, (err) => {
      console.log('Error getting recents', err);
      // this.recents = a.map(i => JSON.parse(i)); //todo: remove
    });
  }

  fetchLastUpdated(): void {
    if (!this.lastUpdated) {
      this.myServiceService.getLastUpdated().subscribe((res: any) => {
